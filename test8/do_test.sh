...
echo "---------------------test8 - файл с префиксом и дописыванием---------------------------"

# Определение названий файлов
INPUT_FILES=("in1.txt")
OUTPUT_FILES=("test_floats.txt" "test_strings.txt" "test_integers.txt")
...
# Запуск JAR-файла
echo "Запуск java -jar $JAR_PATH -a -f -p test_ ${INPUT_FILES[*]}"
java -jar "$JAR_PATH" -a -f -p test_ "${INPUT_FILES[@]}"
if [ $? -ne 0 ]; then
