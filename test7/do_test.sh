...
echo "-----------test7 - 3 файла с полным набором данных и префиксом-------------------"

# Определение названий файлов
INPUT_FILES=("in1.txt" "in2.txt" "in3.txt")
OUTPUT_FILES=("prefix-floats.txt" "prefix-strings.txt" "prefix-integers.txt")
...
# Запуск JAR-файла
echo "Запуск java -jar $JAR_PATH -s -p prefix- ${INPUT_FILES[*]}"
java -jar "$JAR_PATH" -s -p prefix- "${INPUT_FILES[@]}"
if [ $? -ne 0 ]; then
